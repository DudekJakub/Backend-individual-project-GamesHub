...
create trigger GAME_TABLE_AVERAGE_RATING_DELETE_UPDATE
    after delete
    on game_ratings
    for each row
BEGIN
...
create trigger GAME_TABLE_AVERAGE_RATING_INSERT_UPDATE
    after insert
    on game_ratings
    for each row
BEGIN
...
create trigger GAME_TABLE_RATINGS_PER_DAY_DELETE_UPDATE
    after delete
    on game_ratings
    for each row
BEGIN
...
create trigger GAME_TABLE_RATINGS_PER_DAY_INSERT_UPDATE
    after insert
    on game_ratings
    for each row
BEGIN
...
create trigger GAME_TABLE_RATINGS_QNT_DELETE_UPDATE
    after delete
    on game_ratings
    for each row
BEGIN
...
create trigger GAME_TABLE_RATINGS_QNT_INSERT_UPDATE
    after insert
    on game_ratings
    for each row
BEGIN
...
create trigger USER_TABLE_RATINGS_QNT_DELETE_UPDATE
    after delete
    on game_ratings
    for each row
BEGIN
...
create trigger USER_TABLE_RATINGS_QNT_INSERT_UPDATE
    after insert
    on game_ratings
    for each row
BEGIN
...
create trigger GAME_TABLE_OPINIONS_PER_DAY_INSERT_UPDATE
    after insert
    on game_opinions
    for each row
BEGIN
...
create trigger GAME_TABLE_OPINIONS_QNT_DELETE_UPDATE
    after delete
    on game_opinions
    for each row
BEGIN
...
create trigger GAME_TABLE_OPINIONS_QNT_INSERT_UPDATE
    after insert
    on game_opinions
    for each row
BEGIN
...
create trigger GAME_TABLE_OPINION_PER_DAY_DELETE_UPDATE
    after delete
    on game_opinions
    for each row
BEGIN
...
create trigger USER_TABLE_OPINIONS_QNT_DELETE_UPDATE
    after delete
    on game_opinions
    for each row
BEGIN
...
create trigger USER_TABLE_OPINIONS_QNT_INSERT_UPDATE
    after insert
    on game_opinions
    for each row
BEGIN
...
create view opinions_qnt_and_datediff_in_days as
...
create function GameOpinionsPerDay(gameId mediumtext) returns double deterministic
BEGIN
...
create function GameRatingsPerDay(gameId mediumtext) returns double deterministic
BEGIN
    DECLARE result DOUBLE (5,2);
...
create function UserOpinionsPerDay(userId mediumtext) returns double deterministic
BEGIN
    DECLARE result DOUBLE (5,2);
    DECLARE opinions_qnt, opinions_days INT;
...
create function UserRatingsPerDay(userId mediumtext) returns double deterministic
BEGIN
    DECLARE result DOUBLE (5,2);
    DECLARE ratings_qnt, ratings_days INT;
...
create event if not exists CREATE_NEW_GAMES_STATS on schedule
    every '1' HOUR
...
create event if not exists CREATE_NEW_USERS_STATS on schedule
    every '1' HOUR
...
create event if not exists UPDATE_GAMES_OPINIONS_AND_RATING_PER_DAY on schedule
    every '25' SECOND
...
create event if not exists UPDATE_GAMES_POP_STATUSES on schedule
    every '30' SECOND
...
create event if not exists UPDATE_USERS_OPINIONS_AND_RATINGS_PER_DAY on schedule
    every '27' SECOND
